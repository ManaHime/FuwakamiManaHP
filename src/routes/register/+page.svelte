        ...
        <div class="flex flex-col justify-around px-8 pb-8 card gap">
            ...
            <form class="flex flex-col gap-4" method="POST" use:enhance>
                <div class="border border-surface-500 space-y-4 p-7 rounded-container-token">
                    <input class="text-center input rounded-md" name="username" placeholder="ユーザーネーム" type="text" bind:value={username} required>
                    <input class="text-center input rounded-md" name="email" placeholder="email@example.com" type="email" bind:value={email} required>
                    {#if form?.invalid}
                        <p class="error">Username is taken.</p>
                    {/if}
                    <input class="text-center input rounded-md" name="password" placeholder="パスワード" type="password" bind:value={password} required>
                    <input class="text-center input rounded-md" name="confirmPassword" placeholder="もう一回パスワード" type="password" bind:value={confirmPassword} required>
                ...
                <button class="w-full btn variant-filled-primary rounded-md mt-2" id="registerBtn" type="submit" disabled={formDisabled}>Register</button>
            </form>
        </div>
    </div>
</div>