	...
	$: output = days.flatMap(day => 
	  day.streams.length > 0 && day.streams.some(stream => stream.title)
		? day.streams.map(stream => {
			...
			return stream.title ? `<t:${unixTime}:F>: ${stream.title}` : `<t:${unixTime}:F>: No Stream`;
		  })
		...
		<form>
		  {#each days as day, dayIndex}
			<div class="mb-4">
			  ...
			  <button type="button" on:click={() => addStream(dayIndex)} class="btn-sm mt-2 rounded-md variant-filled-primary">Add Stream</button>
			  {#each day.streams as stream, streamIndex}
				<div class="flex">
				  <div>
					<label for={generateId(dayIndex, streamIndex, 'time')}>Time:</label>
					<input type="time" id={generateId(dayIndex, streamIndex, 'time')} bind:value={stream.time} />
				  </div>
				  <div>
					<label for={generateId(dayIndex, streamIndex, 'title')}>Stream Title:</label>
					<input type="text" id={generateId(dayIndex, streamIndex, 'title')} placeholder="Stream" bind:value={stream.title} />
				  </div>
				  {#if day.streams.length > 1}
					<button type="button" on:click={() => removeStream(dayIndex, streamIndex)} class="btn-sm mt-2 rounded-md variant-filled-error">
					  Remove Stream
					</button>
				  {/if}
				</div>
