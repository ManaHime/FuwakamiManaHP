...
<nav class="p-4 list-nav">
    <ul>
        ...
        <li><a href="/portfolio" class="{classesActive("/portfolio")}"  on:click={closeDrawer}>ポートフォリオ</a></li>
        ...
        <li><a href="/register" class="{classesActive("/register")}"  on:click={closeDrawer}>Register</a></li>
        {/if}
        {#if $page.data.user}
            <li><form action="/logout" method="POST"  use:enhance>
